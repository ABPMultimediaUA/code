...
{
	/*this->nodos.push_back(new TNodo(new TTransform())); //at0
	this->nodos.push_back(new TNodo(new TTransform())); //at1
	this->nodos.push_back(new TNodo(new TMalla("models/Nanosuit/nanosuit.obj"))); //at2
	this->nodos.push_back(new TNodo(new TLuz())); //at3
	this->nodos.push_back(new TNodo(new TTransform())); //at4
	this->nodos.push_back(new TNodo(new TCamara())); //at5
	std::cout << "Nodos Creados correctamente" << std::endl;
	this->origen.addHijo(this->nodos.at(1)); //nodo transform hijo de nodo origen
	this->nodos.at(1)->addHijo(this->nodos.at(0)); //nodo transform hijo de nodo transform
	this->nodos.at(0)->addHijo(this->nodos.at(2)); //nodo malla hijo de nodo transform, que a su vez es hijo de otro transform
	this->nodos.at(1)->addHijo(this->nodos.at(3)); //nodo luz hijo de nodo transform, que a su vez es
	...
	this->nodos.push_back(new TNodo(new TTransform())); //at0
	this->nodos.push_back(new TNodo(new TTransform())); //at1
	this->nodos.push_back(new TNodo(new TTransform())); //at2  los hijos de este nodo transform seran todas las mallas que se quieran cargar
	this->nodos.push_back(new TNodo(new TLuz())); //at3
	this->nodos.push_back(new TNodo(new TCamara())); //at4
	this->nodos.push_back(new TNodo(new TMalla("models/Nanosuit/nanosuit.obj"))); //at5
	std::cout << "Nodos Creados correctamente" << std::endl;
	//Ahora anyadimos 3 nodos hijos al nodo origen, que seran los 3 nodos transform
	this->origen.addHijo(this->nodos.at(0)); //nodo transform para la luz
	this->origen.addHijo(this->nodos.at(1)); //nodo transform para la camara
	this->origen.addHijo(this->nodos.at(2)); //nodo transform para la malla
	this->nodos.at(0)->addHijo(this->nodos.at(3)); //nodo luz cuelga del primer nodo transform
	this->nodos.at(1)->addHijo(this->nodos.at(4)); //nodo camara cuelga del segundo nodo transform
	this->nodos.at(2)->addHijo(this->nodos.at(5)); //nodo malla cuelga del tercer nodo transform
	std::cout << "Nodos Enlazados correctamente" << std::endl;
}

void MotorGrafico::recorrerArbolEscena()
{
	//luz
	//nodos.at(3)
}
