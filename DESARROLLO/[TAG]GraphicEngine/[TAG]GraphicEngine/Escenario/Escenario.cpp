...
Escenario::Escenario(TGraphicEngine * motor)
{
	this->motorApp = motor;
}
			...
			for (std::list<ElementoHijo>::iterator T = (*I).ObjetosEscena.begin(); T != (*I).ObjetosEscena.end(); T++) {
				if ((*T).nombre == "Puerta") {
					for (std::list<Elemento>::iterator N = (*T).ObjetosEscena.begin(); N != (*T).ObjetosEscena.end(); N++) {
						...
						transfEM->escalar(
											( (*N).escala.x * (*T).escala.x * (*I).escala.x ),
											( (*N).escala.y * (*T).escala.y * (*I).escala.y ),
											( (*N).escala.z * (*T).escala.z * (*I).escala.z )
										 );
						transfTM->trasladar(
											TRANS * ( (*N).position.x + (*T).position.x + (*I).position.x ),
											TRANS * ( (*N).position.y + (*T).position.y + (*I).position.y ),
											TRANS * ( (*N).position.z + (*T).position.z + (*I).position.z )
										   );
						...
						TMalla* malla = motorApp->crearMalla("resource/models/box/box.obj");
						TNodo* nodoMalla = motorApp->crearNodo(nodoTransfTM, malla);
/*
