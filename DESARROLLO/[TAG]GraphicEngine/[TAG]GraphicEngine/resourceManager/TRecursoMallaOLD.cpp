...
bool TRecursoMalla::cargarFichero(std::string path)
{
	...
	std::vector<TRecursoTextura> textures;
	if(aimesh->mMaterialIndex >= 0)
	{
		aiMaterial* material = scene->mMaterials[aimesh->mMaterialIndex];
		//this->textures_loaded = new std::vector<TRecursoTextura>();
		std::vector<TRecursoTextura> diffuseMaps = loadMaterialTextures(material, aiTextureType_DIFFUSE, "texture_diffuse", &this->textures_loaded, this->directorio);
		textures.insert(textures.end(), diffuseMaps.begin(), diffuseMaps.end());
		std::vector<TRecursoTextura> specularMaps = loadMaterialTextures(material, aiTextureType_SPECULAR, "texture_specular", &this->textures_loaded, this->directorio);
...
std::vector<TRecursoTextura> loadMaterialTextures(aiMaterial * mat, aiTextureType type, std::string typeName, std::vector<TRecursoTextura>* textures_loaded, std::string directorio)
{
	std::vector<TRecursoTextura> textures;
	for (GLuint i = 0; i < mat->GetTextureCount(type); i++)
	{
		...
		if (!skip)
		{
			...
			texture.setID(TextureFromFile(str.C_Str(), directorio));
