...
Escenario::Escenario(ISceneManager* smgr, IVideoDriver* driver) {
   ...
   terrain_node->setPosition(vector3df(0,-10,0));
    ...
    IAnimatedMeshSceneNode *lateral_der = smgr->addAnimatedMeshSceneNode(pared1);
      ...
      IAnimatedMeshSceneNode *objetivo = smgr->addAnimatedMeshSceneNode(pared1);
          objetivo->setMaterialTexture(0, driver->getTexture("texture/pared.bmp"));   
        objetivo->setMaterialFlag(EMF_LIGHTING, false);
      objetivo->setPosition(vector3df(60,0,50));
      objetivo->setRotation(vector3df(-90,0,0));
}
