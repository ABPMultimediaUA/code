...
TRecursoMalla::Mesh::Mesh(const aiMesh *maya, TRecursoMalla *modelo, TGestorRecursos * gr) : buffer{ 0 }, vao{ 0 }, texture_diffuse{ 0 }, texture_specular{ 0 }, texture_ambient{ 0 }, texture_normal{ 0 }
{
	this->model = modelo;
	load(maya, gr);
	create();
...
{
	std::cout << "Ruta de acceso: " << filename << std::endl;
	GLuint textureID = -1;
