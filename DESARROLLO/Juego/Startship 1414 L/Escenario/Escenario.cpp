...
void Escenario::crearLLave(ISceneManager* smgr, IVideoDriver* driver, b2World* world){
    ...
    for(int i = 0; i < 1; i++){
     ...
     /*
        ...
        LLave *lla =  new LLave(smgr, driver, world, vector3df(x,10,z));
       
        llaves.push_back(lla);
...
void Escenario::actualizarListaLLaves(){
    
       if(!llaves.empty()){
        for (std::list<LLave*>::iterator it = llaves.begin(); it != llaves.end();) {
            if ((*it) != NULL) {
                if (!(*it)->entity->getLive()) {
        ...
        for (std::list<LLave*>::iterator it = llaves.begin(); it != llaves.end(); it++) {
            if ((*it) != NULL && (*it)->entity->getLive() == true) {
