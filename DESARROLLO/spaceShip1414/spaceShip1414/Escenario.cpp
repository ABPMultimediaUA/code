...
*/
				...
				if ((*T).nombre == "Paredes") {
					for (std::list<Elemento>::iterator N = (*T).ObjetosEscena.begin(); N != (*T).ObjetosEscena.end(); N++) {
						...
						//	transfTM1->trasladar(0, 0, -((*N).position.z + (*T).position.z + (*I).position.z) );
						//	nodoTransfRM1 = engine->crearNodo(engine->nodoRaiz(), transfRM1);
						//	nodoTransfEM1 = engine->crearNodo(nodoTransfRM1, transfEM1);
						//	nodoTransfTM1 = engine->crearNodo(nodoTransfEM1, transfTM1);
						//	nodoMalla1 = engine->crearNodo(nodoTransfTM1, engine->crearMalla("resourse/models/untitled.obj"));

						//	std::cout << "Posicion x: " << ((*N).position.x + ((*T).position.x + (*I).position.x)) / 10 << std::endl;
						//	std::cout << "Posicion y: " << ((*N).position.y + ((*T).position.y + (*I).position.y)) / 10 << std::endl;
						//	std::cout << "Posicion z: " << ((*N).position.z + ((*T).position.z + (*I).position.z)) / 10 << std::endl;
				...
				if ((*T).nombre == "Suelo") {
					for (std::list<Elemento>::iterator N = (*T).ObjetosEscena.begin(); N != (*T).ObjetosEscena.end(); N++) {
						...
						/*Pared * wall = new Pared(engine);
						wall->Translation()->trasladar((*N).position.x + (*T).position.x + (*I).position.x, 0, -(*N).position.z + (*T).position.z + (*I).position.z);
						wall->Scale()->escalar(((*N).escala.x * (*T).escala.x * (*I).escala.x)*0.1, 1, ((*N).escala.z * (*T).escala.z * (*I).escala.z)*0.1);*/
						//TTransform *transfRM1 = engine->crearTransform();
						//TTransform *transfEM1 = engine->crearTransform();
						//TTransform *transfTM1 = engine->crearTransform();
						//transfEM1->escalar(((*N).escala.x * (*T).escala.x * (*I).escala.x) / 10, ((*N).escala.y * (*T).escala.y * (*I).escala.y) / 10, -((*N).escala.z * (*T).escala.z * (*I).escala.z) / 10);
						//transfTM1->trasladar(((*N).position.x + ((*T).position.x + (*I).position.x))/10, 0, -((*N).position.z + (*T).position.z + (*I).position.z)/10 );
						//TNodo* nodoTransfRM1 = engine->crearNodo(engine->nodoRaiz(), transfRM1);
						//TNodo* nodoTransfEM1 = engine->crearNodo(nodoTransfRM1, transfEM1);
						//TNodo* nodoTransfTM1 = engine->crearNodo(nodoTransfEM1, transfTM1);
						//TNodo* nodoMalla1 = engine->crearNodo(nodoTransfTM1, engine->crearMalla("resourse/models/untitled.obj"));
